...
RUN wget https://github.com/AsamK/signal-cli/releases/download/v0.13.16/signal-cli-0.13.16.tar.gz -O tmp.tar.gz
RUN tar -xvf tmp.tar.gz
RUN mv ./signal-cli-0.13.16/bin/* /usr/local/bin/
RUN mv ./signal-cli-0.13.16/lib/* /usr/local/lib/
RUN rm -rf ./signal-cli-0.13.16
